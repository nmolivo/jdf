...
Transition and rules governing the relationships are not immediately obvious  semantic network representing two states with a transition between them.

To start, transitions rely on a boat which alternates between left and right banks. Sam \includegraphics[scale=0.7]{Figures/sam.png} must take up one place in the boat for each transition because he's the only person capable of maneuvering the boat. Procedurally, all transactions that undo the previous transaction, are invalid. And finally, all transactions which result back to the initial state are invalid as they indicate an inefficiency.

Frodo \includegraphics[scale=0.7]{Figures/frodo.png} and Gollum \includegraphics[scale=0.7]{Figures/gollum.png}Frodo is also the only person capable of watching the ring alone. The anti-relationships that govern the procedures of this problem govern that neither Gollum or Frodo can be alone with the ring at any time. 
